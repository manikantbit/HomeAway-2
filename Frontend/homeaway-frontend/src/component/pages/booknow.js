...
class Book extends Component{
    ...
    bookNow=(values)=>{ 
        ...
        values["propid"] = this.props.match.params.propid;
        values["owneremail"] = this.props.location.state.email;
        ...
        values["price"] = this.props.location.state.price;
        values["amenities"] = this.props.location.state.amenities;
        values["bookedFrom"] = this.props.orderform.availFrom;
        values["bookedTo"] = this.props.orderform.availTo;
        values["nights"] = document.querySelector("input[name=nights]").value;  //this.props.orderform.nights;
        values["createdDate"] = Date.now();
        console.log(values)
        //this.props.bookOrder(values)
        axios.post(`/order`, values)
        .then(response=>{
            if(response.status === 200) {
            this.props.bookOrder(response)
            this.setState({msg:`Order has been booked successfully. Your reference ID is ${response.data.bookid}`})
            }
        })
        .catch(err=>{
        ...
        return (
            ...
            (<div className="alert alert-error fade in">
                ...
                <div className="col-md-6" style={{backgroundColor:'white'}}>
                    ...
                    <h4>Welcome back <strong>{this.props.login.first_name}!</strong></h4>
                    <h4>Signed in as: <strong>{this.props.login.first_name}</strong></h4>
                    </div>
