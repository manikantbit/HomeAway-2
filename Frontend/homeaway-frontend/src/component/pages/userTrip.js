...
class UserTrip extends Component{
    ...
    componentDidMount(){
        ...
        axios.get(`/orders`, {params:{email:email}})
        .then(mybooking=>{
            console.log(mybooking.data)
            this.setState({arrayData: mybooking.data.slice(0,6), itemPerPage:mybooking.data})
            this.props.myTrip(mybooking)
    })
    }
    search = (e) => {
        e.preventDefault()
    ...
    render(){
        ...
        if(this.props.mybooking!=undefined){
