    ...
    componentDidMount(){
        if(this.props.arrayData.length ===0){
            this.setState({msg:"Sorry!! No Results Found. Please check the location value",
                arrayData:this.props.arrayData.slice(0,10),itemPerPage:this.props.arrayData})
        } else {
            this.setState({msg:null,arrayData:this.props.arrayData.slice(0,10),itemPerPage:this.props.arrayData})
    }
}
    search = async (values) => {
        let obj = this;
        ...
        .then(response => {
            ...
            if(response.data.length ===0){
                this.setState({msg:"Sorry!! No Results Found. Please check the location value",
                    arrayData:response.data.slice(0,10),itemPerPage:response.data})
            } else {
                this.setState({msg:null,arrayData:response.data.slice(0,10),itemPerPage:response.data})
        }
        })
